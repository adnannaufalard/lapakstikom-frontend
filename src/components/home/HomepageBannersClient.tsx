...
interface HomepageBannersClientProps {
  banners: Banner[];
}

export function HomepageBannersClient({ banners }: HomepageBannersClientProps) {
  // Filter only promo banners (not HERO type)
  const promoBanners = banners.filter(b => b.banner_type !== 'HERO');

  // Group banners by type
  const promoFull = promoBanners.filter(b => b.banner_type === 'PROMO_FULL');
  const promoLarge = promoBanners.filter(b => b.banner_type === 'PROMO_LARGE');
  const promoSmall = promoBanners.filter(b => b.banner_type === 'PROMO_SMALL');
